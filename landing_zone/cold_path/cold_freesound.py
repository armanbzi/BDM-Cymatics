...
def process_sound(client, sound, audio_np, query):
    ...
    row = {
        "uuid": obs_id,
        "time_recorded": created if created and created != "-" else ts,
...
def run(batch_size=10):
    ...
    # Validate Freesound API key with a tiny probe request
    try:
        probe = search_freesound(QUERIES[0], page_size=1, page=1)
    ...
    # Collect sounds — one random query per record
    collected = []
    seen_ids = set()
    ...
    while len(collected) < batch_size and attempts < max_attempts:
        ...
        # Refill pool for this query if empty
        if not query_pool[query]:
            try:
                results = search_freesound(query, page_size=15, page=query_page[query])
            except requests.exceptions.RequestException as e:
